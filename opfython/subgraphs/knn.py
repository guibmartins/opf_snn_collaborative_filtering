...
class KNNSubgraph(Subgraph):
    """A KNNSubgraph is used to implement a k-nearest neighbours subgraph.
...
class NewKNNSubgraph(Subgraph):
    ...
    def __init__(self, X=None, Y=None, I=None, from_file=None):
        ...
        super(NewKNNSubgraph, self).__init__(X, Y, I, from_file)
    ...
    def calculate_pdf(self, n_neighbours):
        ...
        for i in range(self.n_nodes):
            ...
            # Initialize the number of p.d.f. calculations as 1
            n_pdf += len(self.nodes[i].adjacency[:n_neighbours])

            # Get an array of distances of size k
            distances = self.nodes[i].adj_distances[:n_neighbours]
        ...
        # If subgraph's minimum density is different from the maximum density
        else:
            # For every possible node
            for i in range(self.n_nodes):
    ...
    def create_arcs(self, k, distance_function, pre_computed_distance=False, pre_distances=None):
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(self.n_nodes):
                ...
                # if pre_computed_distance:
                #
                #     dist = pre_distances[self.nodes[i].idx][self.nodes[j].idx]
                #
                # else:
                #
                ...
                dist = distance_function(self.nodes[i].features, self.nodes[j].features)
            ...
            max_dist = float(np.max(i_distances)) if len(i_distances) > 0 else 0.
