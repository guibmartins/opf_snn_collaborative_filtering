...
@d.avoid_zero_division
@njit(cache=True)
def arc_cosine_distance(x, y):
    """Calculates the Arc Cosine Distance.
        ...
        Returns:
            The Arc Cosine Distance between x and y.

        """
    cos = (np.sum(x * y) / (np.sum(x ** 2) ** 0.5 * np.sum(y ** 2) ** 0.5))
...
def shared_near_neighbor_distance(x, y):
    ...
    sim = (len(set(x) & set(y)) + 2.) / float(len(set(x) | set(y)) + 2.)
    return 1. - sim
...
def weighted_voting_shared_neighbors(x, y):
    ...
    shared_neighbors = set(x) & set(y)

    for i in list(shared_neighbors)[:k]:
...
DISTANCES = {
    ...
    'arc_cosine': arc_cosine_distance,
    ...
    'shared_nn': shared_near_neighbor_distance,
    ...
    'weighted_voting_snn': weighted_voting_shared_neighbors
}
